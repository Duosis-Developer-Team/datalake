...
foreach ($ip in $vmwareIps) {
 #   Write-Host "NEW CONNECTİON START"
   ...
   #Write-Host "LOGIN START"
    ...
    ########################################################################################
    #Write-Host "LOGGED IN, CONNECTION START "
 ...
 #   Write-Host "GET DC DONE "
 #   Write-Host $DatacenterTable
    ...
    $vmwareDisconnect = Disconnect-VIServer -Server $serverConnection -Force -Confirm:$false
}
