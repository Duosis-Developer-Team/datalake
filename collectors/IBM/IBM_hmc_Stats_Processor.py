...
class HMC(object):
    def __init__(self,hmc, user, pw):
        ...
        # Optionally set authentication (if basic auth is enough):
        self.session.auth = (user, pw)
        ######################
##################3##############################################################################################################################


    def get_managed_system_data(self):
        self.check_connected("get_managed_system_data")

        url = f"https://{self.HMCname}:12443/rest/api/uom/ManagedSystem"
        headers = {
            'X-API-Session': self.token,
            'Accept': 'application/vnd.ibm.powervm.uom+xml; type=ManagedSystem',
        }

        ret = requests.get(url, headers=headers, verify=False)
        if ret.status_code != 200:
            print(f"Error: GET ManagedSystem data returned code={ret.status_code}")
            print(ret.text)
            return None

        # Save full response to a file for debugging
        with open("newTestOutput.xml", "w") as file:
            file.write(ret.text)

        print("-> Managed System data saved to newTestOutput.xml")
        return ret.text

    # Extract all <id> values from the XML
    def extract_ids_from_xml(self):
        try:
            # Parse the saved XML file
            tree = ET.parse('newTestOutput.xml')
            root = tree.getroot()

            # Lists to store extracted IDs and system names
            id_list = []
            system_name_map = {}
            ...
            for entry in root.findall('.//{http://www.w3.org/2005/Atom}entry'):
                ...
                system_name_elem = entry.find('.//{*}SystemName')

                if id_elem is not None:
                    id_value = id_elem.text
                    id_list.append(id_value)

                    if system_name_elem is not None:
                        system_name_map[id_value] = system_name_elem.text

            print("\n-> Extracted ID and SystemName Values from newTestOutput.xml:")
            for id_value, system_name in system_name_map.items():
                print(f"ID: {id_value}, SystemName: {system_name}")

            return id_list, system_name_map

        except Exception as e:
            print(f"Error extracting IDs and SystemNames from XML: {e}")
            return [], {}

    def fetch_system_details(self, ids, system_name_map):
        system_details = []

        for system_id in ids:
            url = f"https://{self.HMCname}:12443/rest/api/uom/ManagedSystem/{system_id}"
            headers = {
                'X-API-Session': self.token,
                'Accept': 'application/vnd.ibm.powervm.uom+xml; type=ManagedSystem',
            }
            ...
            ret = requests.get(url, headers=headers, verify=False)
            if ret.status_code != 200:
                print(f"Error: Unable to fetch data for ID {system_id}")
                continue

            # Save full XML response to a file
            xml_filename = f"system_detail_{system_id}.xml"

            with open(xml_filename, "w") as xml_file:
                xml_file.write(ret.text)
                print(f"-> Saved raw XML for ID {system_id} to {xml_filename}")
            ...
            # Extract all Uptime values
            for uptime_elem in root.findall('.//{*}Uptime'):  # Match Uptime element regardless of namespace
                uptime_list.append(uptime_elem.text)

            # Collect each system's details
            system_details.append({
                "id": system_id,
                "system_name": system_name_map.get(system_id, "Unknown"),
                "uptimes": uptime_list if uptime_list else ["Unknown"]
            })

            # Print retrieved details for debugging
            print(f"-> Retrieved: ID: {system_id}, SystemName: {system_name_map.get(system_id, 'Unknown')}, Uptimes: {uptime_list}")

        # Save system details to JSON
        with open("system_details_output.json", "w") as json_file:
            json.dump(system_details, json_file, indent=4)
            print("\n-> System details saved to system_details_output.json")



    def cleanup_created_files(self):
        """
        Deletes all files created by the functions.
        """
        # Define patterns for the files created by the functions
        patterns = [
            "newTestOutput.xml",               # File created by get_managed_system_data
            "system_detail_*.xml",            # Files created by fetch_system_details for each ID
            "system_details_output.json"      # File created by fetch_system_details
        ]

        # Iterate through the patterns and delete matching files
